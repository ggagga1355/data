#DEFAULT와 CHECK 제약조건에 대해서
create table TBL_STUDENT(
	id int primary key,
	name varchar(100),
	major varchar(100),
	gender char(1) not null default 'W',#제약조건이 두개이상일 때 나란히 적으면 됨
	birth date,
...
create table tbl_pot(
	...
	constraint flows_pk foreign key (f_name) references tbl_flows(name)
);
...
#공부할땐 편하지만 실제 일할 때는 절대 권장하지 않는다.
...
create table `order`(
	...
	id varchar(100),
	num varchar(100),
	constraint user_pk foreign key (id) references user(id),
	constraint product_pk foreign key (num) references product(p_num)
);

# DML : 데이터를 조작할 수 있는 SQL키워드
...
-- 사원테이블에서 사원의 번호와 이름을 조회
select emp_no, first_name from employees;

-- 급여테이블에서 사원번호와 급여를 조회
...
-- employees테이블에서 성별이 남성인 사원들의 사원과 성을 조회
select emp_no, first_name, last_name 
from employees 
where gender = 'M'; # gender컬럼의 데이터가 'M'과 일치하는 데이터만

-- employees테이블에서 1990년 1월 1일 이후에 입사한 사원들의 사원번호, 이름, 입사일을 조회
# ex) '1990-01-01'
